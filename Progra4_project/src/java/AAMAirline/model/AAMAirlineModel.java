...
//    }
    ...
    public List<Vuelo> getVuelos1() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                vuelo.setDescuento(rs.getString("descuento"));
    ...
    public static int guardar5(Vuelo vu) throws Exception {
        String sql = "insert into " + "vuelo (codigo_vuelo,codigo_ruta,codigo_av,dia_salida,hora_llegada,hora_salida,precio,descuento)"
                + "values('%s','%s','%s','%s','%s','%s','%s','%s')";
        sql = String.format(sql, vu.getCodigo_Vuelo(), vu.getRuta().getCodigo_ruta(), vu.getAvion().getCodigo_Avion(), vu.getDia_salida(), vu.getHora_llegada(), vu.getHora_salida(), vu.getPrecio(),vu.getDescuento());
        ResultSet rs = BD.executeUpdateWithKeys(sql);
        if (rs != null) {
           return 0;
        } else {
